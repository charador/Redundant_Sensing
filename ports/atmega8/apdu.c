...
/** apdu decode for confirmed ack messages*/
uint16_t apdu_decode_confirmed_service_response(
    uint8_t * apdu,     /* APDU data */
    uint16_t apdu_len,
    BACNET_CONFIRMED_SERVICE_ACK_DATA * service_ack_data,
    uint8_t * service_choice,
    uint8_t ** service_ack,
    uint16_t * service_ack_len)
{
    ...
    *service_ack = &apdu[len];
    *service_ack_len = apdu_len - len;
    return len;
}
...
//                len = apdu_decode_confirmed_service_response(&apdu[0],   /* APDU data */
//                    apdu_len, &service_data, &service_choice, &service_request,
//                    &service_request_len);
                /*for RP ack: forward it to the client*/
//                if (service_choice == SERVICE_CONFIRMED_READ_PROPERTY){
//                    oldsrc = get_ori_src();
//                    handler_read_property_ack2(oldsrc, &service_data, &apdu, apdu_len);
//                }
