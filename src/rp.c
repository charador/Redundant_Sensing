...
/*forward RP request to target destination IP*/
int forward_rp_req(BACNET_READ_PROPERTY_DATA * rpdata, uint8_t invoke_id) {
...
/*handler function for rp ack from physical device*/
int rp_handler(void){
    static uint8_t Rx_Buf2[MAX_MPDU] = { 0 };
    unsigned timeout = 100;
    uint16_t pdu_len = 0;
    BACNET_ADDRESS src = {0};  
    fprintf(stderr, "Entering RP_handler function in rp.c.\n");
...
//    int invoke_id = 0; /*for forward rp req*/
    ...
    // forward the request to another device
    invoke_id = forward_rp_req(rpdata, invoke_id);
    rp_handler();
