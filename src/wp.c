...
/*#######added*/
#include "bip.h"
#include "datalink.h" /*data link receive*/
#include "npdu.h" /*for npdu receiver*/
#include "handlers.h"
/*copied from rp main.c*/
#include "arf.h"
#include "tsm.h"
#include "address.h"
#include "npdu.h"
...
/*receiving an WP ack*/
void wp_handler(void){
    static uint8_t Rx_Buf2[MAX_MPDU] = { 0 };
    unsigned timeout = 100;
    uint16_t pdu_len = 0;
    BACNET_ADDRESS src = {0};  
    fprintf(stderr, "Entering wp_handler function in wp.c.\n");
    /* returns 0 bytes on timeout */
    pdu_len = datalink_receive(&src, &Rx_Buf2[0], MAX_MPDU, timeout);

    /* process */
    if (pdu_len) {
        npdu_handler(&src, &Rx_Buf2[0], pdu_len);
    }
}

/*forward WP request to the physical device*/
int forward_wp_request(BACNET_WRITE_PROPERTY_DATA *  wpdata, uint8_t invoke_id){
    BACNET_ADDRESS dest = {0}; // destination IP and port num
    unsigned max_apdu = 0;
    uint32_t device_id = 123; // manually set device_id to be the physical device's ID
//    uint8_t invoke_id = 0;
    bool status = false;
    bool found = false;

    //address binding
    found = address_bind_request(device_id, &max_apdu, &dest);
    if (!found) {
        ...
        if (status) {
            fprintf(stderr, "Found Physical Device ID, now sending WP req...\n");
            invoke_id = Forward_Write_Property_Request_Data(device_id, wpdata->object_type,
        wpdata->object_instance, wpdata->object_property, &wpdata->application_data[0], wpdata->application_data_len,
        wpdata->priority, wpdata->array_index, invoke_id);
            fprintf(stderr, "WP req forwarded to Physical Devices\n");
        }
...
//    int invoke_id = 0;
    ...
    if (apdu_len && wpdata) {
        ...
        /*forward request to the physical device*/
        invoke_id = forward_wp_request(wpdata, invoke_id);
        wp_handler();
    }
